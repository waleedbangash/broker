                ...
                <tbody>
                    @foreach ($services as $service)
                        <tr data-entry-id="{{$service->id }}">
                            ...
                            <td><img src="{{'/'.$service ->service_picture}}" height="40px"/></td>
                            <td>
                            @can('service_edit')
                              <a href="{{route('admin.service.edit',$service->id)}}" class="btn btn-info"><i class="fas fa-pencil-square" aria-hidden="true"></i></a>
                            @endcan
                        </td>
                        <td>
                            @can('service_delete')
...
<script>
  ...
  let deleteButton = {
    ...
    url: "{{route('admin.service.massDestroy')}}",
